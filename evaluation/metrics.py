...
======================
Script đánh giá pipeline — tổ chức lại từ evaluate_phase1.py.
Imports đã được cập nhật sang cấu trúc src.* mới.

Chạy từ thư mục gốc L-RAG/:
    python evaluation/metrics.py --module 1
    python evaluation/metrics.py --module 2
    python evaluation/metrics.py --file data/raw/v1.docx
"""
...
def run_module1(file_path: Path) -> "ingestion.models.LegalDocument":  # type: ignore
    ...
    from src.ingestion.models import LegalDocument
...
def run_module2(doc: "ingestion.models.LegalDocument") -> "list[ingestion.models.LsuChunk]":  # type: ignore
    ...
    from src.ingestion.models import NodeType, ContentType
...
def run_module3(
    ...
    from src.ingestion.knowledge_store import HybridGraphBuilder, placeholder_embedding_fn
...
Ví dụ:
  ...
  python evaluation/metrics.py --file docs_test/v1.pdf
  python evaluation/metrics.py --file data_test/01-tand_signed_v1.docx
  python evaluation/metrics.py --module 1
  python evaluation/metrics.py --module 2
  python evaluation/metrics.py --query "quyền của bên trong hợp đồng"
  python evaluation/metrics.py --dry-run
        """,
    )
    parser.add_argument(
        "--file",
